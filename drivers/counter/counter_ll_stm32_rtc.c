...
LOG_MODULE_REGISTER(counter_rtc_stm32);
				...
				const struct counter_alarm_cfg *alarm_cfg)
{
	...
	DEV_DATA(dev)->alarm_cfg = alarm_cfg;
...
static int rtc_stm32_disable_alarm(struct device *dev,
				  const struct counter_alarm_cfg *alarm_cfg)
{
	if (DEV_DATA(dev)->alarm_cfg != alarm_cfg) {
		LOG_ERR("Invalid alarm");
		return -EINVAL;
	}
...
static u32_t rtc_stm32_get_wrap(struct device *dev)
{
	const struct counter_config_info *info = dev->config->config_info;

	return info->max_wrap;
}


static int rtc_stm32_set_wrap(struct device *dev, u32_t ticks,
...
void rtc_stm32_isr(void *arg)
{
	...
	const struct counter_alarm_cfg *alarm_cfg = DEV_DATA(dev)->alarm_cfg;
	...
	if (LL_RTC_IsActiveFlag_ALRA(RTC) != 0) {
		...
		if (alarm_cfg && alarm_cfg->handler) {
			alarm_cfg->handler(dev, alarm_cfg, now);
		}
	}
...
static const struct rtc_stm32_config rtc_config = {
	...
	.pclken = {
		.enr = CONFIG_RTC_0_CLOCK_BITS,
		.bus = CONFIG_RTC_0_CLOCK_BUS,
	},
	.ll_rtc_config = {
...
static const struct counter_driver_api rtc_stm32_driver_api = {
		...
		.set_wrap = rtc_stm32_set_wrap,
		.get_pending_int = rtc_stm32_get_pending_int,
		.get_wrap = rtc_stm32_get_wrap,
