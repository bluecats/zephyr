...
static int tim_stm32_set_top_value(struct device *dev,   
									const struct counter_top_cfg *cfg)
{
	
	LL_TIM_SetAutoReload(DEV_CFG(dev)->timer, cfg->ticks);
...
#define COUNTER_TIM_STM32_DEVICE(idx)	\
	...
	{ \
		...
		cfg->timer = (TIM_TypeDef *)DT_TIMER_STM32_##idx##_BASE_ADDRESS; \
