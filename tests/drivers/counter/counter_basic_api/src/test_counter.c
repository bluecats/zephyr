...
#define ALARM_US (2*USEC_PER_SEC)
#else
#define ALARM_US (0.5*WRAP_PERIOD_US)
#endif
...
const char *devices[] = {
	...
	CONFIG_COUNTER_1_NAME,
#endif
#ifdef CONFIG_COUNTER_TIMER2
	CONFIG_COUNTER_2_NAME,
#endif
#ifdef CONFIG_COUNTER_TIMER3
	CONFIG_COUNTER_3_NAME,
#endif
...
void test_single_shot_alarm_instance(const char *dev_name)
{
	...
	dev = device_get_binding(dev_name);
	ticks = counter_us_to_ticks(dev, ALARM_US);
